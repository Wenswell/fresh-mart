<template>
  <div>
    ...
    <!-- 搜索栏 定位 -->
    <van-sticky>
      <van-search class="top-search-bar" v-model="searchStr" show-action shape="round" placeholder="未搜索时按默认顺序加载所有商品"
        @search="onSearch" @cancel="$router.push('/layout/home')">
        <!-- @search="onSearch" @cancel="$router.back()"> -->
      </van-search>

      <van-dropdown-menu active-color="#1989fa">
        <van-dropdown-item v-model="params1" @change="dropDownChange" :options="option1" />
        <van-dropdown-item v-model="params2" @change="dropDownChange" :options="option2" />
      </van-dropdown-menu>
    ...
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
      <van-grid :column-num="2" :border="false">
        <van-grid-item class="product-div" v-for="item in searchResult" :key="item.id"
  ...
  methods: {
    ...
    onLoad() {
      ...
      LOCALsearchProductApi({ 'keyword': this.searchStr, 'page': ++this.currentPage }).then(res => {
        ...
        this.loading = false
        // 加载完成
        if (this.searchResult.length == res.result.pageData.counts) this.finished = true;
      })
